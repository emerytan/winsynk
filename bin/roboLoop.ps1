...
for ($i = 0; $i -lt $shotsPaths.Length; $i++) {
    ...
    Robocopy.exe $shotsPaths[$i] $destination /l /nfl /ndl /njh /e /xd (Get-ChildItem $excludeListPath | Where-Object {$_.Name -Match $(Split-Path $shotsPaths[$i] -Leaf)} | Get-Content) (Get-Content $plateExcludes) /xf '.DS_Store', 'Thumbs.db' /w:0 /r:0
    Start-Sleep -Seconds 5
}
